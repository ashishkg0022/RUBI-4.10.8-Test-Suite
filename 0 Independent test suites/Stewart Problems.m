...
{x^(-1), x, 1, Log[x]}
{a^x, x, 1, a^x/Log[a]}
{Sec[x]*Tan[x], x, 1, Sec[x]}
{Cot[x]*Csc[x], x, 1, -Csc[x]}
{ArcTan[x], x, 2, x*ArcTan[x] - Log[1 + x^2]/2}
{E^(2*x)*x, x, 2, -E^(2*x)/4 + (E^(2*x)*x)/2}
{x*Cos[x], x, 2, Cos[x] + x*Sin[x]}
{x*Sin[4*x], x, 2, -(x*Cos[4*x])/4 + Sin[4*x]/16}
{x*Log[x], x, 2, -x^2/4 + (x^2*Log[x])/2}
...
{t*Cos[t]*Sin[t], t, 2, -t/4 + (Cos[t]*Sin[t])/4 + (t*Sin[t]^2)/2}
{t*Sec[t]^2, t, 2, Log[Cos[t]] + t*Tan[t]}
{t^2*Log[t], t, 2, -t^3/9 + (t^3*Log[t])/3}
{E^t*t^3, t, 4, -6*E^t + 6*E^t*t - 3*E^t*t^2 + E^t*t^3}
...
{t/E^t, t, 2, -E^(-t) - t/E^t}
{Sqrt[t]*Log[t], t, 2, (-4*t^(3/2))/9 + (2*t^(3/2)*Log[t])/3}
...
{E^x^2*x^3, x, 2, -E^x^2/2 + (E^x^2*x^2)/2}
{E^x*(3 + 2*x), x, 2, -2*E^x + E^x*(3 + 2*x)}
...
{x^5*Cos[x^3], x, 2, Cos[x^3]/3 + (x^3*Sin[x^3])/3}
{E^x^2*x^5, x, 3, E^x^2 - E^x^2*x^2 + (E^x^2*x^4)/2}
{E^Sqrt[x], x, 3, -2*E^Sqrt[x] + 2*E^Sqrt[x]*Sqrt[x]}
{x*Cos[Pi*x], x, 2, Cos[Pi*x]/Pi^2 + (x*Sin[Pi*x])/Pi}
{Sqrt[x]*Log[x], x, 2, (-4*x^(3/2))/9 + (2*x^(3/2)*Log[x])/3}
{Sin[3*x]^2, x, 1, x/2 - (Cos[3*x]*Sin[3*x])/6}
{Cos[x]^2, x, 1, x/2 + (Cos[x]*Sin[x])/2}
{Cos[x]^4, x, 2, (3*x)/8 + (3*Cos[x]*Sin[x])/8 + (Cos[x]^3*Sin[x])/4}
{Sin[x]^3, x, 2, -Cos[x] + Cos[x]^3/3}
{Cos[x]^4*Sin[x]^3, x, 3, -Cos[x]^5/5 + Cos[x]^7/7}
{Cos[x]^3*Sin[x]^4, x, 3, Sin[x]^5/5 - Sin[x]^7/7}
{Cos[x]^2*Sin[x]^4, x, 3, x/16 + (Cos[x]*Sin[x])/16 - (Cos[x]^3*Sin[x])/8 - (Cos[x]^3*Sin[x]^3)/6}
{Cos[x]^2*Sin[x]^2, x, 2, x/8 + (Cos[x]*Sin[x])/8 - (Cos[x]^3*Sin[x])/4}
{(1 - Sin[2*x])^2, x, 2, (3*x)/2 + Cos[2*x] - (Cos[2*x]*Sin[2*x])/4}
{Cos[x]*Sin[Pi/6 + x], x, 3, x/4 - Cos[Pi/6 + 2*x]/4}
{Cos[x]^5*Sin[x]^5, x, 4, Sin[x]^6/6 - Sin[x]^8/4 + Sin[x]^10/10}
{Sin[x]^6, x, 3, (5*x)/16 - (5*Cos[x]*Sin[x])/16 - (5*Cos[x]*Sin[x]^3)/24 - (Cos[x]*Sin[x]^5)/6}
{Sqrt[Cos[x]]*Sin[x]^3, x, 3, (-2*Cos[x]^(3/2))/3 + (2*Cos[x]^(7/2))/7}
{x*Sin[x^2]^3, x, 3, -Cos[x^2]/2 + Cos[x^2]^3/6}
{Sin[x]^2*Tan[x], x, 3, Cos[x]^2/2 - Log[Cos[x]]}
{Cos[x]^2*Cot[x]^3, x, 3, -Csc[x]^2/2 - 2*Log[Sin[x]] + Sin[x]^2/2}
...
{Sec[x]^4, x, 2, Tan[x] + Tan[x]^3/3}
{Sec[x]^2*Tan[x]^4, x, 1, Tan[x]^5/5}
{Sec[x]^4*Tan[x]^2, x, 3, Tan[x]^3/3 + Tan[x]^5/5}
{Sec[x]^3*Tan[x], x, 1, Sec[x]^3/3}
...
{Csc[2*x]*(Cos[x] + Sin[x]), x, 6, -ArcTanh[Cos[x]]/2 + ArcTanh[Sin[x]]/2}
...
{x/Sqrt[3 - 2*x - x^2], x, 2, -Sqrt[3 - 2*x - x^2] + ArcSin[(-1 - x)/2]}
...
{x^3/Sqrt[4 + x^2], x, 2, (-8*Sqrt[4 + x^2])/3 + (x^2*Sqrt[4 + x^2])/3}
{1/Sqrt[9 + x^2], x, 1, ArcSinh[x/3]}
{Sqrt[1 + x^2], x, 2, (x*Sqrt[1 + x^2])/2 + ArcSinh[x]/2}
{1/(x^3*Sqrt[-16 + x^2]), x, 4, Sqrt[-16 + x^2]/(32*x^2) + ArcTan[Sqrt[-16 + x^2]/4]/128}
{Sqrt[-a^2 + x^2]/x^4, x, 1, (-a^2 + x^2)^(3/2)/(3*a^2*x^3)}
{Sqrt[-4 + 9*x^2]/x, x, 3, Sqrt[-4 + 9*x^2] - 2*ArcTan[Sqrt[-4 + 9*x^2]/2]}
...
{1/(x*Sqrt[3 + x^2]), x, 2, -(ArcTanh[Sqrt[3 + x^2]/Sqrt[3]]/Sqrt[3])}
...
{Sqrt[2*x - x^2], x, 3, -Sqrt[2*x - x^2]/2 + (x*Sqrt[2*x - x^2])/2 - ArcSin[1 - x]/2}
{1/Sqrt[8 + 4*x + x^2], x, 1, ArcSinh[(2 + x)/2]}
{1/Sqrt[-8 + 6*x + 9*x^2], x, 2, ArcTanh[(1 + 3*x)/Sqrt[-8 + 6*x + 9*x^2]]/3}
{x^2/Sqrt[4*x - x^2], x, 3, -3*Sqrt[4*x - x^2] - (x*Sqrt[4*x - x^2])/2 - 6*ArcSin[1 - x/2]}
{(2 + 2*x + x^2)^(-2), x, 2, (1 + x)/(2*(2 + 2*x + x^2)) + ArcTan[1 + x]/2}
{(5 - 4*x - x^2)^(-5/2), x, 2, (2 + x)/(27*(5 - 4*x - x^2)^(3/2)) + (2*(2 + x))/(243*Sqrt[5 - 4*x - x^2])}
{E^t*Sqrt[9 - E^(2*t)], t, 3, (E^t*Sqrt[9 - E^(2*t)])/2 + (9*ArcSin[E^t/3])/2}
{Sqrt[-9 + E^(2*t)], t, 4, Sqrt[-9 + E^(2*t)] - 3*ArcTan[Sqrt[-9 + E^(2*t)]/3]}
{1/Sqrt[a^2 + x^2], x, 2, ArcTanh[x/Sqrt[a^2 + x^2]]}
...
{(2 - 3*x + 4*x^2)/(3 - 4*x + 4*x^2), x, 4, x + ArcTan[(1 - 2*x)/Sqrt[2]]/(4*Sqrt[2]) + Log[3 - 4*x + 4*x^2]/8}
{(1 + x^2 + x^3)/((-1 + x)*x*(1 + x^2)^3*(1 + x + x^2)), x, 13, (1 + x)/(8*(1 + x^2)^2) - (3*(1 - x))/(8*(1 + x^2)) + (3*x)/(16*(1 + x^2)) + (7*ArcTan[x])/16 - ArcTan[(1 + 2*x)/Sqrt[3]]/Sqrt[3] + Log[1 - x]/8 - Log[x] + (15*Log[1 + x^2])/16 - Log[1 + x + x^2]/2}
{(1 - 3*x + 2*x^2 - x^3)/((1 + x^2)^2*z), x, 8, (2 - x)/(2*(1 + x^2)*z) + (3*ArcTan[x])/(2*z) - Log[1 + x^2]/(2*z)}
{(1 + x^2)^(-2), x, 2, x/(2*(1 + x^2)) + ArcTan[x]/2}
{1/((-1 + x)*(2 + x)), x, 2, Log[1 - x]/3 - Log[2 + x]/3}
...
{(-x^3 + x^4)^(-1), x, 3, 1/(2*x^2) + x^(-1) + Log[1 - x] - Log[x]}
...
{(19*x)/((-1 + x)^3*(3 + 5*x + 4*x^2)^2), x, 7, -399/(736*(1 - x)^2) - 1843/(4416*(1 - x)) + (19*(39 + 44*x))/(276*(1 - x)^2*(3 + 5*x + 4*x^2)) + (114437*ArcTan[(5 + 8*x)/Sqrt[23]])/(52992*Sqrt[23]) + (209*Log[1 - x])/2304 - (209*Log[3 + 5*x + 4*x^2])/4608}
{(1 + x^2 + x^3)/(2*x^2 + x^3 + x^4), x, 6, -1/(2*x) + ArcTan[(1 + 2*x)/Sqrt[7]]/(4*Sqrt[7]) - Log[x]/4 + (5*Log[2 + x + x^2])/8}
{(-x^3 + x^6)^(-1), x, 7, 1/(2*x^2) - ArcTan[(1 + 2*x)/Sqrt[3]]/Sqrt[3] + Log[1 - x]/3 - Log[1 + x + x^2]/6}
{x^2/(1 + x), x, 2, -x + x^2/2 + Log[1 + x]}
{x/(-5 + x), x, 2, x + 5*Log[5 - x]}
{(-1 + 4*x)/((-1 + x)*(2 + x)), x, 2, Log[1 - x] + 3*Log[2 + x]}
{1/((1 + x)*(2 + x)), x, 2, Log[1 + x] - Log[2 + x]}
{(-5 + 6*x)/(3 + 2*x), x, 2, 3*x - 7*Log[3 + 2*x]}
{1/((a + x)*(b + x)), x, 2, -(Log[a + x]/(a - b)) + Log[b + x]/(a - b)}
...
{(-x^2 + x^4)^(-1), x, 3, x^(-1) - ArcTanh[x]}
{(-x + 2*x^3)/(1 - x^2 + x^4), x, 3, Log[1 - x^2 + x^4]/2}
{x^3/(1 + x^2), x, 2, x^2/2 - Log[1 + x^2]/2}
{(-1 + x)/(2 + 2*x + x^2), x, 3, -2*ArcTan[1 + x] + Log[2 + 2*x + x^2]/2}
{x/(1 + x + x^2), x, 3, -(ArcTan[(1 + 2*x)/Sqrt[3]]/Sqrt[3]) + Log[1 + x + x^2]/2}
{(7 + 5*x + 4*x^2)/(5 + 4*x + 4*x^2), x, 4, x + (3*ArcTan[1/2 + x])/8 + Log[5 + 4*x + 4*x^2]/8}
{(5 - 4*x + 3*x^2)/((-1 + x)*(1 + x^2)), x, 5, -3*ArcTan[x] + 2*Log[1 - x] + Log[1 + x^2]/2}
{(3 + 2*x)/(3*x + x^3), x, 6, (2*ArcTan[x/Sqrt[3]])/Sqrt[3] + Log[x] - Log[3 + x^2]/2}
{(-1 + x^3)^(-1), x, 5, -(ArcTan[(1 + 2*x)/Sqrt[3]]/Sqrt[3]) + Log[1 - x]/3 - Log[1 + x + x^2]/6}
{x^3/(1 + x^3), x, 6, x + ArcTan[(1 - 2*x)/Sqrt[3]]/Sqrt[3] - Log[1 + x]/3 + Log[1 - x + x^2]/6}
...
{(-3 + x)/(4 + 2*x + x^2)^2, x, 2, -(7 + 4*x)/(6*(4 + 2*x + x^2)) - (2*ArcTan[(1 + x)/Sqrt[3]])/(3*Sqrt[3])}
...
{(-3 + 2*x + x^2)^(-1), x, 3, Log[1 - x]/4 - Log[3 + x]/4}
{(-2*x + x^2)^(-1), x, 3, Log[2 - x]/2 - Log[x]/2}
{(1 + 2*x)/(-7 + 12*x + 4*x^2), x, 3, Log[1 - 2*x]/8 + (3*Log[7 + 2*x])/8}
{x/(-1 + x + x^2), x, 3, ((5 - Sqrt[5])*Log[1 - Sqrt[5] + 2*x])/10 + ((5 + Sqrt[5])*Log[1 + Sqrt[5] + 2*x])/10}
{(-32 + 5*x - 27*x^2 + 4*x^3)/(-70 - 299*x - 286*x^2 + 50*x^3 - 13*x^4 + 30*x^5), x, 5, (3988*ArcTan[(1 + 2*x)/Sqrt[19]])/(13685*Sqrt[19]) - (3146*Log[7 - 3*x])/80155 - (334*Log[1 + 2*x])/323 + (4822*Log[2 + 5*x])/4879 + (11049*Log[5 + x + x^2])/260015}
{(8 - 13*x^2 - 7*x^3 + 12*x^5)/(4 - 20*x + 41*x^2 - 80*x^3 + 116*x^4 - 80*x^5 + 100*x^6), x, 8, 5828/(9075*(2 - 5*x)) - (313 + 502*x)/(1452*(1 + 2*x^2)) - (251*ArcTan[Sqrt[2]*x])/(726*Sqrt[2]) + (272*Sqrt[2]*ArcTan[Sqrt[2]*x])/1331 - (59096*Log[2 - 5*x])/99825 + (2843*Log[1 + 2*x^2])/7986}
{Sqrt[4 + x]/x, x, 3, 2*Sqrt[4 + x] - 4*ArcTanh[Sqrt[4 + x]/2]}
{(-x^(-1/3) + Sqrt[x])^(-1), x, 11, 2*Sqrt[x] + 6*Sqrt[2/(5*(5 + Sqrt[5]))]*ArcTan[(1 - Sqrt[5] + 4*x^(1/6))/Sqrt[2*(5 + Sqrt[5])]] - (3*Sqrt[2*(5 + Sqrt[5])]*ArcTan[(Sqrt[(5 + Sqrt[5])/10]*(1 + Sqrt[5] + 4*x^(1/6)))/2])/5 + (6*Log[1 - x^(1/6)])/5 - (3*(1 + Sqrt[5])*Log[2 + (1 - Sqrt[5])*x^(1/6) + 2*x^(1/3)])/10 - (3*(1 - Sqrt[5])*Log[2 + (1 + Sqrt[5])*x^(1/6) + 2*x^(1/3)])/10}
{(-4*Cos[x] + 3*Sin[x])^(-1), x, 2, -ArcTanh[(3*Cos[x] + 4*Sin[x])/5]/5}
{(1 + Sqrt[x])^(-1), x, 2, 2*Sqrt[x] - 2*Log[1 + Sqrt[x]]}
{(1 + x^(-1/3))^(-1), x, 5, 3*x^(1/3) - (3*x^(2/3))/2 + x - 3*Log[1 + x^(1/3)]}
{Sqrt[x]/(1 + x), x, 3, 2*Sqrt[x] - 2*ArcTan[Sqrt[x]]}
{1/(x*Sqrt[1 + x]), x, 2, -2*ArcTanh[Sqrt[1 + x]]}
{(-x^(1/3) + x)^(-1), x, 2, (3*Log[1 - x^(2/3)])/2}
{(x - Sqrt[2 + x])^(-1), x, 4, (4*Log[2 - Sqrt[2 + x]])/3 + (2*Log[1 + Sqrt[2 + x]])/3}
...
{x^3/(1 + x^2)^(1/3), x, 2, (-9*(1 + x^2)^(2/3))/20 + (3*x^2*(1 + x^2)^(2/3))/10}
{Sqrt[x]/(-x^(-1/3) + Sqrt[x]), x, 12, 6*x^(1/6) + x - (3*Sqrt[2*(5 + Sqrt[5])]*ArcTan[(1 - Sqrt[5] + 4*x^(1/6))/Sqrt[2*(5 + Sqrt[5])]])/5 - (3*Sqrt[2*(5 - Sqrt[5])]*ArcTan[(Sqrt[(5 + Sqrt[5])/10]*(1 + Sqrt[5] + 4*x^(1/6)))/2])/5 + (6*Log[1 - x^(1/6)])/5 - (3*(1 - Sqrt[5])*Log[2 + (1 - Sqrt[5])*x^(1/6) + 2*x^(1/3)])/10 - (3*(1 + Sqrt[5])*Log[2 + (1 + Sqrt[5])*x^(1/6) + 2*x^(1/3)])/10}
{(x^(-1/4) + Sqrt[x])^(-1), x, 8, 2*Sqrt[x] + (4*ArcTan[(1 - 2*x^(1/4))/Sqrt[3]])/Sqrt[3] + (4*Log[1 + x^(1/4)])/3 - (2*Log[1 - x^(1/4) + Sqrt[x]])/3}
{(x^(-1/3) + x^(-1/4))^(-1), x, 17, 12*x^(1/12) - 6*x^(1/6) + 4*x^(1/4) - 3*x^(1/3) + (12*x^(5/12))/5 - 2*Sqrt[x] + (12*x^(7/12))/7 - (3*x^(2/3))/2 + (4*x^(3/4))/3 - (6*x^(5/6))/5 + (12*x^(11/12))/11 - x + (12*x^(13/12))/13 - (6*x^(7/6))/7 + (4*x^(5/4))/5 - 12*Log[1 + x^(1/12)]}
{Sqrt[(1 - x)/x], x, 5, Sqrt[-1 + x^(-1)]*x - ArcTan[Sqrt[-1 + x^(-1)]]}
{Cos[x]/(Sin[x] + Sin[x]^2), x, 4, Log[Sin[x]] - Log[1 + Sin[x]]}
...
{(3 - 5*Sin[x])^(-1), x, 4, -Log[Cos[x/2] - 3*Sin[x/2]]/4 + Log[3*Cos[x/2] - Sin[x/2]]/4}
{(Cos[x] + Sin[x])^(-1), x, 2, -(ArcTanh[(Cos[x] - Sin[x])/Sqrt[2]]/Sqrt[2])}
{(1 - Cos[x] + Sin[x])^(-1), x, 2, -Log[1 + Cot[x/2]]}
{(4*Cos[x] + 3*Sin[x])^(-1), x, 2, -ArcTanh[(3*Cos[x] - 4*Sin[x])/5]/5}
{(Sin[x] + Tan[x])^(-1), x, 5, -ArcTanh[Cos[x]]/2 + (Cot[x]*Csc[x])/2 - Csc[x]^2/2}
{(2*Sin[x] + Sin[2*x])^(-1), x, 4, Log[Tan[x/2]]/4 + Tan[x/2]^2/8}
{Sec[x]/(1 + Sin[x]), x, 5, ArcTanh[Sin[x]]/2 - 1/(2*(1 + Sin[x]))}
{(b*Cos[x] + a*Sin[x])^(-1), x, 2, -(ArcTanh[(a*Cos[x] - b*Sin[x])/Sqrt[a^2 + b^2]]/Sqrt[a^2 + b^2])}
{(b^2*Cos[x]^2 + a^2*Sin[x]^2)^(-1), x, 2, ArcTan[(a*Tan[x])/b]/(a*b)}
...
{x/Sqrt[1 - x^2], x, 1, -Sqrt[1 - x^2]}
{x^3*Log[x], x, 2, -x^4/16 + (x^4*Log[x])/4}
...
{Sec[x]^4*Tan[x]^3, x, 3, Tan[x]^4/4 + Tan[x]^6/6}
{x/(2 - 2*x + x^2), x, 3, -ArcTan[1 - x] + Log[2 - 2*x + x^2]/2}
{x*ArcSin[x], x, 3, (x*Sqrt[1 - x^2])/4 - ArcSin[x]/4 + (x^2*ArcSin[x])/2}
{Sqrt[9 - x^2]/x, x, 3, Sqrt[9 - x^2] - 3*ArcTanh[Sqrt[9 - x^2]/3]}
{x/(2 + 3*x + x^2), x, 3, -Log[1 + x] + 2*Log[2 + x]}
{x^2*Cosh[x], x, 3, -2*x*Cosh[x] + 2*Sinh[x] + x^2*Sinh[x]}
{(1 + x + x^3)/(4*x + 2*x^2 + x^4), x, 1, Log[x*(4 + 2*x + x^3)]/4}
...
{(1 + x + x^2 + x^3)^(-1), x, 6, ArcTan[x]/2 + Log[1 + x]/2 - Log[1 + x^2]/4}
...
{x^2*ArcTan[x], x, 3, -x^2/6 + (x^3*ArcTan[x])/3 + Log[1 + x^2]/6}
{(1 - Sqrt[x])/x^(1/3), x, 2, (3*x^(2/3))/2 - (6*x^(7/6))/7}
{(-E^(-x) + E^x)^(-1), x, 2, -ArcTanh[E^x]}
{x/(10 + 2*x^2 + x^4), x, 2, ArcTan[1/3 + x^2/3]/6}
{(x^(-1/3) + x)^(-1), x, 2, (3*Log[1 + x^(4/3)])/4}
{Cos[x]^4*Sin[x]^2, x, 3, x/16 + (Cos[x]*Sin[x])/16 + (Cos[x]^3*Sin[x])/24 - (Cos[x]^5*Sin[x])/6}
{1/Sqrt[5 - 4*x - x^2], x, 1, -ArcSin[(-2 - x)/3]}
{x/(1 - x^2 + Sqrt[1 - x^2]), x, 3, -Log[1 + Sqrt[1 - x^2]]}
{(1 + Cos[x])*Csc[x], x, 1, Log[1 - Cos[x]]}
{E^x/(-1 + E^(2*x)), x, 2, -ArcTanh[E^x]}
{(-8 + x^3)^(-1), x, 5, -ArcTan[(1 + x)/Sqrt[3]]/(4*Sqrt[3]) + Log[2 - x]/12 - Log[4 + 2*x + x^2]/24}
...
{x/((1 + x^2)*(4 + x^2)), x, 3, Log[1 + x^2]/6 - Log[4 + x^2]/6}
{(4 - 5*Sin[x])^(-1), x, 4, -Log[Cos[x/2] - 2*Sin[x/2]]/3 + Log[2*Cos[x/2] - Sin[x/2]]/3}
{x*(c + x)^(1/3), x, 2, (-3*c*(c + x)^(4/3))/4 + (3*(c + x)^(7/3))/7}
{E^x^(1/3), x, 4, 6*E^x^(1/3) - 6*E^x^(1/3)*x^(1/3) + 3*E^x^(1/3)*x^(2/3)}
{(4 + x + Sqrt[1 + x])^(-1), x, 4, (-2*ArcTan[(1 + 2*Sqrt[1 + x])/Sqrt[11]])/Sqrt[11] + Log[4 + x + Sqrt[1 + x]]}
...
{(x + Sin[x])^2, x, 5, x/2 + x^3/3 - 2*x*Cos[x] + 2*Sin[x] - (Cos[x]*Sin[x])/2}
{E^ArcTan[x]/(1 + x^2), x, 1, E^ArcTan[x]}
{1/(x*(1 + x^4)), x, 1, Log[x] - Log[1 + x^4]/4}
{t^3/E^(2*t), t, 4, -3/(8*E^(2*t)) - (3*t)/(4*E^(2*t)) - (3*t^2)/(4*E^(2*t)) - t^3/(2*E^(2*t))}
{Sqrt[t]/(1 + t^(1/3)), t, 6, -6*t^(1/6) + 2*Sqrt[t] - (6*t^(5/6))/5 + (6*t^(7/6))/7 + 6*ArcTan[t^(1/6)]}
{Sin[x]*Sin[2*x]*Sin[3*x], x, 5, -Cos[2*x]/8 - Cos[4*x]/16 + Cos[6*x]/24}
{Log[x/2], x, 1, -x + x*Log[x/2]}
{Sqrt[(1 + x)/(1 - x)], x, 3, (-2*Sqrt[(1 + x)/(1 - x)])/(1 + (1 + x)/(1 - x)) + 2*ArcTan[Sqrt[(1 + x)/(1 - x)]]}
{(x*Log[x])/Sqrt[-1 + x^2], x, 4, -Sqrt[-1 + x^2] + ArcTan[Sqrt[-1 + x^2]] + Sqrt[-1 + x^2]*Log[x]}
{(a + x)/(a^2 + x^2), x, 3, ArcTan[x/a] + Log[a^2 + x^2]/2}
{Sqrt[1 + x - x^2], x, 2, -((1 - 2*x)*Sqrt[1 + x - x^2])/4 - (5*ArcSin[(1 - 2*x)/Sqrt[5]])/8}
{x^4/(16 + x^10), x, 2, ArcTan[x^5/4]/20}
{(2 + x)/(2 + x + x^2), x, 3, (3*ArcTan[(1 + 2*x)/Sqrt[7]])/Sqrt[7] + Log[2 + x + x^2]/2}
{x*Sec[x]*Tan[x], x, 2, -ArcTanh[Sin[x]] + x*Sec[x]}
{x/(-a^4 + x^4), x, 2, -ArcTanh[x^2/a^2]/(2*a^2)}
{(Sqrt[x] + Sqrt[1 + x])^(-1), x, 3, (-2*x^(3/2))/3 + (2*(1 + x)^(3/2))/3}
{(1 - E^(-x) + 2*E^x)^(-1), x, 4, Log[1 - 2*E^x]/3 - Log[1 + E^x]/3}
{ArcTan[Sqrt[x]]/Sqrt[x], x, 3, 2*Sqrt[x]*ArcTan[Sqrt[x]] - Log[1 + x]}
{Log[1 + x]/x^2, x, 3, Log[x] - Log[1 + x] - Log[1 + x]/x}
{(-E^x + E^(3*x))^(-1), x, 3, E^(-x) - ArcTanh[E^x]}
...
{x*Sqrt[4 + 2*x + x^2], x, 3, -((1 + x)*Sqrt[4 + 2*x + x^2])/2 + (4 + 2*x + x^2)^(3/2)/3 - (3*ArcSinh[(1 + x)/Sqrt[3]])/2}
...
{Sqrt[4 - 3*x^2]/x, x, 3, Sqrt[4 - 3*x^2] - 2*ArcTanh[Sqrt[4 - 3*x^2]/2]}
...
{x^2*Cos[3*x], x, 3, (2*x*Cos[3*x])/9 - (2*Sin[3*x])/27 + (x^2*Sin[3*x])/3}
{Sqrt[5 - 4*x - x^2], x, 2, ((2 + x)*Sqrt[5 - 4*x - x^2])/2 - (9*ArcSin[(-2 - x)/3])/2}
{x^5/(Sqrt[2] + x^2), x, 3, -(x^2/Sqrt[2]) + x^4/4 + Log[Sqrt[2] + x^2]}
{Sec[x]^5, x, 3, (3*ArcTanh[Sin[x]])/8 + (3*Sec[x]*Tan[x])/8 + (Sec[x]^3*Tan[x])/4}
{Sin[2*x]^6, x, 3, (5*x)/16 - (5*Cos[2*x]*Sin[2*x])/32 - (5*Cos[2*x]*Sin[2*x]^3)/48 - (Cos[2*x]*Sin[2*x]^5)/12}
{Cos[x]*Log[Sin[x]]*Sin[x]^2, x, 3, -Sin[x]^3/9 + (Log[Sin[x]]*Sin[x]^3)/3}
...
{Sin[x]^4, x, 2, (3*x)/8 - (3*Cos[x]*Sin[x])/8 - (Cos[x]*Sin[x]^3)/4}
